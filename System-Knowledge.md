...
| `generate-personalized-email` | Personalisierte Email-Generierung (Gemini 2.5 Flash, structured output). Akzeptiert `offer`-Block (Rabatt, Gutschein, Gültigkeit, Hinweis) sowie `intent`/`tone`/`length` für Absichts- und Stilsteuerung und liefert ein `variants[]`-Array (mehrere Vorschläge mit Label, Betreff, Inhalt inkl. Signatur, abwärtskompatibel). Anti-Halluzinations-Regeln und ANGEBOTSDETAILS-Block bleiben übergeordnet. Details: `docs/Guest-Personalization-Improvement-Concept.md` |
