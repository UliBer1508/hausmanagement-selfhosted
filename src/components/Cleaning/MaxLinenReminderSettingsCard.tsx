 ...
 * - max_linen_reminder_enabled: An/Aus-Schalter (Not-Aus, falls Amela es zu viel wird)
 * - max_linen_reminder_days_before: wie viele Tage vor der Reinigung Max fragt
 */
const MaxLinenReminderSettingsCard = () => {
  ...
  useEffect(() => {
    if (settings) {
      setLocalEnabled(settings.max_linen_reminder_enabled ?? false);
      setLocalDaysBefore(settings.max_linen_reminder_days_before ?? 5);
      setHasChanges(false);
    }
  }, [settings]);
  ...
  const handleSave = () => {
    updateSettings({
      max_linen_reminder_enabled: localEnabled,
      max_linen_reminder_days_before: localDaysBefore,
    });
    setHasChanges(false);
  };
        ...
        <CollapsibleTrigger className="w-full">
          <CardHeader className="flex flex-row items-center justify-between">
            <div className="flex items-center gap-2 text-left">
              ...
              <div>
                <CardTitle className="text-base">Max: Wäsche-Erinnerungen an Teuni</CardTitle>
                <CardDescription className="text-sm">
                  Max erinnert Teuni automatisch, die Wäsche rechtzeitig vor der Reinigung zu liefern
                </CardDescription>
              </div>
            </div>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex items-center justify-between rounded-lg border p-4">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">
                  {localEnabled
                    ? 'Max erinnert Teuni selbstständig an anstehende Wäsche-Lieferungen.'
                    : 'Max ist still – es werden keine Erinnerungen an Teuni gesendet.'}
                </p>
              </div>
              <Switch
            ...
            <div className="rounded-lg border p-4 space-y-3">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">
                  Wie viele Tage vor der Reinigung soll Max Teuni erinnern? Bespreche den Wert mit Teuni.
                </p>
              </div>
...
export default MaxLinenReminderSettingsCard;
