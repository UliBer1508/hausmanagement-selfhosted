...
> **Status: VERBINDLICH (Muss).** Gilt für **alle KI-Agenten** bei jeder
...
1. **`docs/CODE-INDEX.md` lesen** und die Zieldatei über Tab → Kette → Doppelgänger
...
11. **Doku nachziehen im selben Commit:** betroffene Datei in `docs/CODE-INDEX.md`
...
Damit dieser Standard tatsächlich befolgt wird:

1. Alle Pflichtdokumente liegen in `docs/`: `CODE-INDEX.md`, `CODING-GUIDE.md`,
   `PROJEKT-REGELN.md`, `ARBEITSWEISE-CLAUDE-LESSONS.md`.
   `AGENTS.md` bleibt im Repo-Root (wird von KI-Werkzeugen dort gesucht) und
   verweist auf die übrigen.
2. Bei Aufgaben an eine KI genügt der Verweis „nach CODING-GUIDE/CODE-INDEX
   arbeiten" — beide Dateien werden dann zuerst herangezogen.
3. **Wichtig:** Ein Teil der Geschäftslogik liegt in **DB-Triggern**
   (`supabase/SQL/`), nicht im TypeScript-Code. Wer nur den Code liest, übersieht
   die Hälfte der Wirkung.

> **Lovable wird nicht mehr genutzt** (Stand 13.07.2026). Code-Änderungen laufen
> über den GitHub-Editor oder lokal.
