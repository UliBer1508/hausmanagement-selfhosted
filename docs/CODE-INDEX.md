...
angelegt. Sie sind gleichwertig zu Buchungsreinigungen: Termin, Dienstleister,
Stunden, Terminfrage.
...
**Kein eigener `service_type`.** Das Enum hat nur `cleaning` und `laundry`.
Fensterreinigung ist fachlich eine Reinigung und wird wie eine behandelt — auch
abrechnungsseitig (Stundensatz des Dienstleisters). Kommt Fensterputzen zu einer
Buchungsreinigung dazu, sind das schlicht mehr `cleaning_hours` auf demselben
