...
## 18. Geplante Erweiterungen (Konzepte, noch nicht umgesetzt)

### Zusatzkosten bei Buchungsänderung
> Detail-Konzept: `docs/Booking-Surcharges-Concept.md`

Wenn sich eine Buchung ändert (z. B. Personenzahl von 2 → 3), sollen die zusätzlich
in Rechnung zu stellenden Beträge (Bettwäsche pro Person, Ortstaxe pro Person/Nacht)
automatisch berechnet werden.

- **Heutiger Stand:** Nebenkosten je Haus existieren (`AdditionalFeesTab.tsx`,
  `usePricingConfig.ts`, `houses.additional_fees` / Tabelle `house_additional_fees`),
  aber **nur als Aufenthalts-Pauschalen** und **ohne Delta-Berechnung** bei Änderung.
- **Geplant:** pro-Person-Sätze, serverseitige Delta-Engine
  (`calculate-booking-delta`), neue Tabelle `booking_charges` (Einzelforderungen).
- **Bekannte Inkonsistenz:** `houses.additional_fees` (JSONB, flach gelesen) vs.
  Tabelle `house_additional_fees` (mit `platform`) vs. ursprüngliche JSONB-Segmentierung
  (`booking_com`/`airbnb`) — Konsolidierung empfohlen.

### Stripe-Integration (Zahlungslinks & Online-Zahlung)
> Detail-Konzept: `docs/Stripe-Integration-Concept.md`

- **Heutiger Stand:** **Kein Stripe-Code** im Repository vorhanden.
- **Geplant:** Edge Functions `create-payment-link` + `stripe-webhook`, zentrale
  Tabelle `payments`, kanalübergreifend nutzbar für Buchungs-Zusatzforderungen,
  Direktbuchung über die eigene Website und perspektivisch Wallbox-Abrechnung.
- **Empfehlung:** kein separates DB-Environment — eigene Tabellen im bestehenden
  Supabase-Projekt; Kartendaten ausschließlich bei Stripe (PCI-Scope vermeiden).
